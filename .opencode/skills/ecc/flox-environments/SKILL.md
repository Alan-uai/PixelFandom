...
description: "Create reproducible, cross-platform development environments with Flox — a declarative environment manager built on Nix. ALWAYS use this skill when the user needs system-level dependencies (compilers, databases, native libraries); configure reproducible toolchains for Python, Node.js, Rust, Go, C/C++, Java, Ruby, Elixir, PHP; manage environments identically across macOS and Linux; pin exact package versions; run local services (PostgreSQL, Redis, Kafka) alongside dev tools; onboard developers with one command; or solve 'works on my machine' problems. Also use when the user mentions .flox/, manifest.toml, flox activate, or FloxHub."
origin: Flox
---
