...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    const drawCell = (img: HTMLImageElement | null, i: number, title: string) => {
      const x = (i % this.atlasSize) * cellSize;
      const y = Math.floor(i / this.atlasSize) * cellSize;

      if (img && img.width > 0) {
        // cover-fit the image into the square cell
        ...
        const hue = hashHue(title || `item-${i}`);
        ...
        ctx.font = `700 ${cellSize * 0.5}px sans-serif`;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText((title || '?').charAt(0).toUpperCase(), x + cellSize / 2, y + cellSize / 2);
      }
    };
    ...
    Promise.all(
      ...
      images.forEach((img, i) => drawCell(img, i, this.items[i].title));
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, onNavigate }) => {
    ...
    <div className="infinite-wikis-root">
      ...
      {failed && (
        <div className="infinite-wikis-fallback-note">
          Seu navegador não suporta WebGL2. Veja as wikis abaixo.
        </div>
      )}
