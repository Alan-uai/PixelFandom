...
export const probabilityDetector: ShapeDetector = {
  ...
  render({ value, useSuffix }: DetectionContext, variant = 1): ReactNode {
    ...
    const colorClass = pct >= 75 ? 'text-primary' : pct >= 40 ? 'text-amber-500' : pct >= 15 ? 'text-orange-500' : 'text-red-500';
    const bgClass = pct >= 75 ? 'bg-primary/10' : pct >= 40 ? 'bg-amber-500/10' : pct >= 15 ? 'bg-orange-500/10' : 'bg-red-500/10';

    if (variant === 2) {
      return (
        <div className={`inline-flex items-center gap-1.5 rounded-full ${bgClass} border px-2.5 py-1 text-xs font-medium ${colorClass}`}>
          <span>{formatNumber(pct, !!useSuffix)}%</span>
    ...
    if (variant === 3) {
      return (
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className={`font-mono font-bold ${colorClass}`}>{formatNumber(pct, !!useSuffix)}%</span>
        </div>
      );
    }
    if (variant === 4) {
      return (
        <div className="flex items-center gap-2 rounded-lg border bg-card p-2 text-xs">
          <div className="relative h-8 w-8">
            ...
            <span className="absolute inset-0 flex items-center justify-center text-[8px] font-bold">{formatNumber(pct, !!useSuffix)}%</span>
          </div>
      ...
      return (
        <div className={`rounded-lg border ${bgClass} ${colorClass} p-2.5 text-xs space-y-1`}>
          <div className="flex items-center justify-between">
            ...
            <span className="font-bold text-sm">{formatNumber(pct, !!useSuffix)}%</span>
          </div>
