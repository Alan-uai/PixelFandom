...
export const priceDetector: ShapeDetector = {
  ...
  render({ value }: DetectionContext, variant = 1): ReactNode {
    ...
    const displayPrice = currency ? `${price} ${currency}` : String(price ?? '—');
    ...
    if (variant === 2) {
      return (
        <div className="flex items-center gap-2 rounded-lg border bg-card p-2 text-xs">
          ...
          {discount != null && (
            <span className="text-[10px] text-emerald-500 bg-emerald-500/10 rounded-full px-1.5">-{discount}%</span>
          )}
        </div>
      );
    }
        ...
        <div className="inline-flex items-center gap-1.5 rounded-full border bg-primary/5 px-3 py-1 text-xs font-medium">
          ...
          {discount != null && discountedPrice != null ? (
            <>
              ...
              <span className="font-bold text-emerald-500">{discountedPrice} {currency}</span>
            </>
          ) : (
            <span className="font-bold text-foreground">{displayPrice}</span>
          )}
      ...
      return (
        <div className="flex items-center justify-between rounded-lg border bg-card p-2.5 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`font-bold ${discount ? 'text-emerald-500' : 'text-foreground'}`}>
              {discount && discountedPrice != null ? `${discountedPrice} ${currency}` : displayPrice}
            </span>
          </div>
        </div>
      );
      ...
      return (
        <div className="flex items-center gap-2 rounded-xl border-2 border-primary/20 bg-card p-3 text-sm">
          ...
          <div>
            ...
            {discount != null && (
              <span className="text-[10px] text-emerald-500 ml-2">-{discount}% OFF</span>
            )}
          </div>
        </div>
      );
