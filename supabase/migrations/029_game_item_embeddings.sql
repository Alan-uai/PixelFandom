-- Migration 029: Add embedding VECTOR(1536) to game tables for semantic search
-- Also recreate search_all with semantic search support

-- =====================================================
-- 1. Add embedding column to game tables
-- =====================================================
DO $$
DECLARE
  tbl TEXT;
  tables TEXT[] := ARRAY['weapons', 'armors', 'enemies', 'bosses', 'rings', 'potions', 'upgrades'];
BEGIN
...
-- =====================================================
-- 2. Recreate search_all with semantic search support
-- =====================================================
...
BEGIN
  ...
  search_weapons AS (
    ...
    SELECT 'weapon' AS source_type, id, name,
      ...
      NULL::text[] AS tags, 'Weapons' AS collection_name, 'weapons' AS collection_slug,
      row_to_json(weapons.*)::jsonb AS raw_data,
      COALESCE((1 - (embedding <=> v_embedding_vector)) * 10, 0)::int AS rank,
      'semantic' AS match_type, updated_at
    FROM weapons
