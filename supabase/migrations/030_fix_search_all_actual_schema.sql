...
BEGIN
    ...
    RETURN QUERY
    SELECT sub.source_type, sub.id, sub.title, sub.slug, sub.summary, sub.content, sub.raw_data, sub.rank, sub.image_url, sub.tags, sub.item_stats
    FROM (
    WITH
    search_wiki AS (
    ...
    WHERE sub.rank > 0
    ORDER BY sub.rank DESC, sub.source_type
    LIMIT p_limit;
END;
$BODY$;
